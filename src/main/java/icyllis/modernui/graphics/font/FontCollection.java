...
import calebxzhou.lucidfont.LucidFont;
...
public class FontCollection {
    ...
    static {
        ...
        try (InputStream stream = new FileInputStream(LucidFont.getFontFileInConfig())) {
            Font font = Font.createFont(Font.TRUETYPE_FONT, stream);
            fonts.add(font);
        } catch (FontFormatException | IOException e) {
			e.printStackTrace();
			throw new IllegalArgumentException("错误，无法读取RDI字体文件，您正确安装rdi核心了吗？？");
        }
        ...
        // no backup strategy
        SANS_SERIF = DEFAULT;//new FontCollection(new Font[]{sansSerif});
