...
public class TextRenderType extends RenderType {
	...
	public static synchronized void preloadShaders() {
		...
		final var fallback = Minecraft.getInstance().getVanillaPackResources().asProvider();
		final var provider = (ResourceProvider) location -> {
			...
			return Optional.of(new Resource(new InMemoryResourcePack.Named(""), () -> stream));
		};
		try {
			sShader = new ShaderInstance(provider, SHADER_RL.getPath(),
					DefaultVertexFormat.POSITION_COLOR_TEX_LIGHTMAP);
			sShaderGlow = new ShaderInstance(provider, SHADER_GLOW_RL.getPath(),
					DefaultVertexFormat.POSITION_COLOR_TEX_LIGHTMAP);
			sShaderSeeThrough = new ShaderInstance(provider, SHADER_SEE_THROUGH_RL.getPath(),
					DefaultVertexFormat.POSITION_COLOR_TEX_LIGHTMAP);
		} catch (IOException e) {
